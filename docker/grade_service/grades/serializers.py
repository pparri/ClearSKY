...
class QuestionGradeSerializer(serializers.ModelSerializer):
    class Meta:
        model = QuestionGrade
        fields = ['id', 'question_number', 'value']
...
class GradeAssignmentSerializer(serializers.ModelSerializer):
    ...
    course_title = serializers.CharField(source='course_title', read_only=True)
    course_code = serializers.CharField(source='course_code', read_only=True)


    class Meta:
        model = GradeAssignment
        fields = [
            'id', 'student', 'course_id', 'course_title', 'course_code',
            'grade_value', 'state', 'semester', 'question_grades', 'timestamp'
        ]
        read_only_fields = ['timestamp', 'instructor']
